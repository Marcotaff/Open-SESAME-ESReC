...
fil_name = 'test'        # name to the file to which the daily input profiles will be stored
...
days = np.size(input_profile,1) # number of days you want to simulate
bsize = 52                      # size of the battery in the vehicle (kWh)


# CELL PARAMETERS
        
Q0 = 1.5                   # cell capacity in Ah
soc_min = 0                # minimum soc during cycling
soc_max = 1                # maximum soc during cycling
v_max = 4.2                # maximum voltage during cycling
v_min = 2.7838             # minimum voltage during cycling
soc = soc_in = 0.6         # minimum initial SoC
...
for j in range(days):
    ...
    prof[:] = input_profile[:,j]/bsize          # copy basic daily profiles developed above into another array (for final adjustments)
    t1 = 1 / 3600                               # converting seconds to hours (required by model)
    sf_tempcal = 0.0875 * exp(0.0556*temp1)     # calculating stress factor for temperature (calendar aging)
    ...
    for i in range(len(prof)):
        ...
        sf_soccal = 0.0077*soc_lookup + 0.2525          # calendar aging SoC stress factor calculation
    ...
    soc1=soc_g[0]       # hack to avoid the last points not being counted
    soc2=soc_g[-1]
    soc_g[-1]=0
    soc_g[0]=1                  
    y=-1*soc_g
    ...
    a.sort()
    sf_temp = 0.0008*temp1**2 - 0.033*temp1 + 0.8349   
    
    for i in range(len(a)-1):
        dod = abs(soc_g[a[i]]-soc_g[a[i+1]])*100                # dod computation for cycle aging
        fec = dod/2/100                                         # full equivalent cycles in current step
        tfec = tfec + fec                                       # total full equivalent cycles
        cr = (soc_g[a[i+1]]-soc_g[a[i]])/((a[i+1]-a[i])/3600)   # c-rate in current step
        msoc = (soc_g[a[i]]+soc_g[a[i+1]])/2                    # mean soc in current step
        ...
        Qcyc+=ref_cyc * sf_temp * sf_dod * sf_cr * sf_soc * fec     # adding cycle aging in current step to cumulative cycle aging
        sor+= ref_sor * sf_cr_sor * sf_dod_sor * sf_soc_sor * fec   # adding sor increase in current step to cumulative sor increase
...
print('Time required for the simulation was '+ elapsed + ' mins')
print('The calendar and cycle aging are '+ str(Qcal) + ' and ' + str(Qcyc) + ' respectively')
print('The SoR is ' + str(sor))
print('Full equivalent cycles are ' + str(ceil(tfec)))